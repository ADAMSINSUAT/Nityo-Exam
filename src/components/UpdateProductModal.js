...
export default function UpdateProductModal({products, openUpdateProductModal, closeUpdateProductModal, updateProductID }) {
    //For controlling the visibility of the update product modal
    const [openModal, setOpenModal] = React.useState(false);

    //To find the values of the product to be updated
    let product = products.find(item => item.id === updateProductID);

    //For properly removing the image
    const inputRef = React.useRef(); // import useRef from react
    ...
    React.useEffect(() => {
        ...
        if (product !== undefined) {
            ...
            if (openUpdateProductModal) {
                setOpenModal(true);
            }

            if (name === "" && unit === "" && Math.trunc(price) === 0 && available_inventory === 0 && image === null && imageName === "") {
                setName(product.name);
                setUnit(product.unit);
                setPrice(product.price);
                setDate_Of_Expiry(product.date_of_expiry);
                setAvailable_Inventory(product.available_inventory);
                setImage(product.image);
                setImageName(product.image_name);
            }
        }

    }, [formValidationAlert, openUpdateProductModal, product])
    ...
    function submit(e) {
        ...
        if ((validateName(name) && validateUnit(unit) && validatePrice(price) && validateAvailableInventory(available_inventory)) === true && image !== null) {
            ...
            let productIndex = inventory.findIndex(item => item.id === updateProductID);
            products[productIndex] = { ...products[productIndex], name: name, unit: unit, price: price, available_inventory: available_inventory, date_of_expiry: date_of_expiry, image: image, image_name: imageName };
            hideModal();
        }
        else {
        ...
        >
            <Box sx={{
                ...
                <Stack alignContent="flex-start" direction="row" spacing={55}>
                    <Typography variant="h4" sx={{ marginBottom: 5 }}>Update product:</Typography>
                ...
                <form onSubmit={submit}>
                    ...
                    <Grid item container direction="column" spacing={2} xs={6}>
                        ...
                        <Grid item><Button variant="contained" type="submit">Update Product</Button>
                        </Grid>
                    </Grid>
                </form>
            </Box>
